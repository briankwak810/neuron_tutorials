    ...
    if sim.allSimData['spkt'] == []:
        ...
        for i in range(1, 10 + 1):
            spike_times_i = [
                ...
                if 100*(i-2) + initial_spike_a <= spike_id <= 100*(i-1) + initial_spike_a
            ]
...
spkvar, spka = run_single_packet_w(50, 3, 50, 7)
spkvar2, spka2 = run_single_packet_w(50, 3, 52, 7)
spkvar3, spka3 = run_single_packet_w(50, 1, 57, 7)
spkvar4, spka4 = run_single_packet_w(10, 5, 95, 7)
spkvar5, spka5 = run_single_packet_w(8, 5, 90, 17)
...
for i in range(10, 100, 10):
    # Plot and add arrows for spka
    spkvar, spka = run_single_packet_w(40, 5, i, 7)
    a_in = spka[:-1]
    a_out = spka[1:]
...
# Plot and add arrows for spka2
a_in2 = spka2[:-1]
a_out2 = spka2[1:]
plt.plot(a_in2, a_out2, '-o', markersize=4, color='green', label='spka2')
for i in range(len(a_in2) - 1):
    plt.annotate('',
                 xy=(a_in2[i+1], a_out2[i+1]),
                 xytext=(a_in2[i], a_out2[i]),
...
a_in3 = spka3[:-1]
a_out3 = spka3[1:]
plt.plot(a_in3, a_out3, '-o', markersize=4, color='red', label='spka3')
for i in range(len(a_in3) - 1):
    plt.annotate('',
                 xy=(a_in3[i+1], a_out3[i+1]),
                 xytext=(a_in3[i], a_out3[i]),
                 arrowprops=dict(arrowstyle='->', color='red', lw=1),
                 )
    
a_in4 = spka4[:-1]
a_out4 = spka4[1:]
plt.plot(a_in4, a_out4, '-o', markersize=4, color='pink', label='spka4')
for i in range(len(a_in4) - 1):
    plt.annotate('',
                 xy=(a_in4[i+1], a_out4[i+1]),
                 xytext=(a_in4[i], a_out4[i]),
                 arrowprops=dict(arrowstyle='->', color='pink', lw=1),
                 )
    
a_in5 = spka5[:-1]
a_out5 = spka5[1:]
plt.plot(a_in5, a_out5, '-o', markersize=4, color='orange', label='spka5')
for i in range(len(a_in5) - 1):
    plt.annotate('',
                 xy=(a_in5[i+1], a_out5[i+1]),
                 xytext=(a_in5[i], a_out5[i]),
...
plt.xlabel('$a_{in}$ (spikes)', fontsize=12)
